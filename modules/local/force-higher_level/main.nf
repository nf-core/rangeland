process FORCE_HIGHER_LEVEL {
    ...
    def smaEndmember       = task.ext.args?.getAt("SMA_ENDMEMBER")         ? "SMA_ENDMEMBER = ${task.ext.args["SMA_ENDMEMBER"]}"                 : "SMA_ENDMEMBER = 1"
    def smaOutputRms       = task.ext.args?.getAt("OUTPUT_RMS")            ? "OUTPUT_RMS = ${task.ext.args["OUTPUT_RMS"]}"                       : "OUTPUT_RMS = FALSE"

    // Interpolation parameters
    def interpolateMethod  = task.ext.args?.getAt("INTERPOLATE")           ? "INTERPOLATE = ${task.ext.args["INTERPOLATE"]}"                     :"INTERPOLATE = NONE"
    def movingMax          = task.ext.args?.getAt("MOVING_MAX")            ? "MOVING_MAX = ${task.ext.args["MOVING_MAX"]}"                       : "MOVING_MAX = 16"
    ...
    def polAdaptive        = task.ext.args?.getAt("POL_ADAPTIVE")          ? "POL_ADAPTIVE = ${task.ext.args["POL_ADAPTIVE"]}"                   : "POL_ADAPTIVE = TRUE"
    def pol                = task.ext.args?.getAt("POL")                   ? "POL = ${task.ext.args["POL"]}"                                     : "POL = VSS VPS VES VSA RMR IGS"
    ...
    def outputPOL          = task.ext.args?.getAt("OUTPUT_POL")            ? "OUTPUT_POL = ${task.ext.args["OUTPUT_POL"]}"                       : "OUTPUT_POL = FALSE"
