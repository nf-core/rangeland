...
workflow RANGELAND {
    ...
    //
    def data           = channel.empty()
    def dem            = channel.empty()
    def wvdb           = channel.empty()
    def cube_file      = file( params.data_cube )
    def aoi_file       = file( params.aoi )
    def endmember_file = params.endmember ? file( params.endmember ) : []
    def aod            = params.aod       ? file ( params.aod   )    : []
    def coreg          = params.coreg     ? file ( params.coreg )    : []
